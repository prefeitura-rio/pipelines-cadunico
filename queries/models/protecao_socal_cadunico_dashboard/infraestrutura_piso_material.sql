...
SELECT
  ...
  ROUND(100 * COUNT(DISTINCT id_familia) / SUM(COUNT(DISTINCT id_familia)) OVER(PARTITION BY data_particao), 2) AS porcentagem_material_piso_domicilio,
  COUNT(DISTINCT id_familia) numero_familias,
  SUM(COUNT(DISTINCT id_familia)) OVER(PARTITION BY data_particao) AS total_familias,
FROM tb
GROUP BY data_particao,  id_cras_creas, material_piso_domicilio
ORDER BY id_cras_creas, data_particao, material_piso_domicilio